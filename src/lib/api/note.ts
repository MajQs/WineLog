...
import type { CreateNoteCommand, NoteDto, NoteListResponseDto, MessageResponseDto } from "@/types";
 ...
 */
export async function createNote(batchId: string, command: CreateNoteCommand): Promise<NoteDto> {
 ...
 */
export async function deleteNote(batchId: string, noteId: string): Promise<MessageResponseDto> {
  return fetchWithAuth<MessageResponseDto>(`/api/v1/batches/${batchId}/notes/${noteId}`, {
    method: "DELETE",
  });
}
