...
function StageItem({
  ...
  const lineColor = isCompleted ? "bg-green-600" : isCurrent ? "bg-blue-600" : "bg-gray-300";
  ...
  return (
    <div className="relative flex gap-4 pb-8 group">
      ...
      {!isLast && (
        <div
          className={cn("absolute left-[15px] top-8 w-0.5 h-full -ml-px transition-colors", lineColor)}
          aria-hidden="true"
        />
      )}
      ...
      <div className="flex-1 pt-0.5">
        <div className="flex items-center gap-2 mb-1">
          <span className="text-xs font-medium text-gray-500">Etap {stage.position}</span>
          {isCurrent && (
            <span className="text-xs font-semibold text-blue-600 bg-blue-100 px-2 py-0.5 rounded-full">Aktualny</span>
          )}
        </div>
        ...
        <div className="flex flex-col gap-1 text-xs text-gray-500">
          {stage.started_at && <span>Rozpoczęto: {new Date(stage.started_at).toLocaleDateString("pl-PL")}</span>}
          {stage.completed_at && <span>Ukończono: {new Date(stage.completed_at).toLocaleDateString("pl-PL")}</span>}
...
export function StageTimeline({ stages, currentStageId, isArchived }: StageTimelineProps) {
  ...
  return (
    <div className="space-y-0" role="list" aria-label="Oś czasu etapów produkcji">
