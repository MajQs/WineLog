...
function groupNotesByStage(notes: NoteDto[], stages: BatchStageDto[]) {
  ...
  for (const [stageId, stageNotes] of grouped) {
    grouped.set(
      stageId,
      stageNotes.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
    );
  }
      ...
      {mostRecentNote && mostRecentGroup && (
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2 border-b pb-2">
            ...
            <h4 className="text-sm font-medium text-gray-700">{mostRecentGroup.stageInfo?.description}</h4>
          </div>
            ...
            return (
              <div key={stageId} className="space-y-3">
                ...
                <div className="flex items-center gap-2 border-b pb-2">
                  ...
                  <h4 className="text-sm font-medium text-gray-700">{stageInfo?.description}</h4>
                  <span className="text-muted-foreground ml-auto text-xs">
                    {stageNotes.length}{" "}
                    {stageNotes.length === 1 ? "notatka" : stageNotes.length < 5 ? "notatki" : "notatek"}
                  </span>
                </div>
